...
def main(
    model="./spacy_trf_zh",
    new_model_name="zh_bert_ner",
    output_dir="./zh_bert_ner",
    ...
    if "trf_ner" not in nlp.pipe_names:
        ner = nlp.create_pipe("trf_ner")
        nlp.add_pipe(ner, last=True)
    # otherwise, get it, so we can add labels to it
    else:
        ner = nlp.get_pipe("trf_ner")
    ...
    pipe_exceptions = ["trf_ner", "trf_wordpiecer", "trf_tok2vec"]
    other_pipes = [
